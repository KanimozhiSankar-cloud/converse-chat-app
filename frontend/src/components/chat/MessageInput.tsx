...
import { FilePlus2, Gift, Laugh, Paperclip, Send, SmilePlus, X } from 'lucide-react';
  ...
  return (
    <div className="bg-page px-4 pb-4 pt-3 md:px-7 md:pb-6">
      ...
      <form ref={composerRef} onSubmit={handleSubmit} className="glass-panel relative flex items-end gap-1.5 rounded-xl p-1.5">
        ...
        >
          {isSending ? <Spinner size="sm" className="border-ink-950 border-t-transparent" /> : <Send className="h-4 w-4" />}
        </button>
      </form>
    </div>
  );
